...
class Args:
    ...
    # --- OMEGA goal sampling (v7) ---
    use_omega: bool = False           # master flag; off => v6 behavior bit-exact
    omega_b: float = -3.0             # bias b_Omega in omega_coeff = 1 / max(b + KL, 1); matches mrl/configs/continuous_off_policy.py:curiosity_beta=-3.0. With b=-3, omega_coeff stays clamped at 1 until KL>4, then drops
...
@struct.dataclass
class OmegaState:
    """State for OMEGA goal sampling (v7). See plans/algorithms/reppo_gcrl_algorithm_v7.md."""
...
def train(args: Args):
    ...
    # See plans/algorithms/reppo_gcrl_algorithm_v7.md for the spec these mirror.
    if args.use_omega:
        ...
        C_omega_08 = (1.0 - args.gamma**int(0.8 * H_omega)) / (1.0 - args.gamma)
        c_omega_init = float(C_omega_full - 3.0)
        c_omega_floor = float(C_omega_full - C_omega_08)
        omega_C_B = args.omega_K_B * args.num_envs * args.num_steps
        ...
        def mega_select(rng, x, omega_state, train_state):
            """Pseudocode MegaSelect, vectorized over envs.
            ...
            viable = q >= omega_state.cutoff
            has_viable = viable.any(axis=-1)
            argmax_q = jnp.argmax(q, axis=-1)
            fallback = jax.nn.one_hot(argmax_q, N).astype(jnp.bool_)
        ...
        def update_omega_cutoff(omega_state, just_completed_mask):
            ...
            new_cutoff_min = jnp.where(
                any_completed,
                jnp.maximum(c_omega_floor, jnp.minimum(batch_qmin, omega_state.cutoff_min)),
            ...
            cutoff_low = jnp.maximum(
                jnp.minimum(omega_state.cutoff + delta, c_omega_init), c_omega_floor
            )
            adapt = new_count >= 10
    ...
    def collect_rollout(key, train_state: ReppoTrainingState):
        ...
        def step_env_omega(carry, _):
            ...
            # Keep a chosen goal for the duration of an episode segment: only
            # adopt the freshly-proposed one when the previous step ended an
            # episode (or on the very first scan step, which init seeds with
            # prev_done=True for every env).
            ...
            obs_with_chosen = replace_goal_in_obs(env_state.obs, chosen_goal)
            action = policy(act_key, obs_with_chosen, train_state)
            action = jnp.clip(action, -1 + 1e-4, 1 - 1e-4)
            next_env_state = env.step(env_state, action)

            # Achieved goal lives in goal_indices, which is disjoint from the
            # last goal_dim (desired-goal) slot we wrote chosen_goal into.
            raw_next_obs = next_env_state.info["raw_obs"]
            achieved = raw_next_obs[..., goal_indices]
            reward_omega = compute_goal_reward(raw_next_obs, chosen_goal)
            hit = (reward_omega > 0).astype(jnp.float32)
            ...
            transition = RolloutTransition(
                ...
                action=action,
                reward=reward_omega,
                next_obs=replace_goal_in_obs(raw_next_obs, chosen_goal),
                done=next_env_state.done,
                truncated=next_env_state.info["truncation"],
                extras={
                    **next_env_state.info,
